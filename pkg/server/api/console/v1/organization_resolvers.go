...
// IcpmsDocuments is the resolver for the icpmsDocuments field.
func (r *organizationResolver) IcpmsDocuments(ctx context.Context, obj *types.Organization, first *int, after *page.CursorKey, last *int, before *page.CursorKey, orderBy *types.IcpmsDocumentOrderBy, filter *types.IcpmsDocumentFilter) (*types.IcpmsDocumentConnection, error) {
	scope, err := r.authorize(ctx, obj.ID, probo.ActionDocumentList)
	if err != nil {
		return nil, err
	}

	coreFilter := coredata.NewIcpmsDocumentFilter()

	if filter != nil {
		if filter.Query != nil {
			coreFilter.WithQuery(*filter.Query)
		}
		if len(filter.Types) > 0 {
			var types []coredata.IcpmsDocumentType
			for _, t := range filter.Types {
				types = append(types, coredata.IcpmsDocumentType(t))
			}
			coreFilter.WithTypes(types...)
		}
		if len(filter.Groups) > 0 {
			var groups []coredata.IcpmsDocumentGroup
			for _, g := range filter.Groups {
				groups = append(groups, coredata.IcpmsDocumentGroup(g))
			}
			coreFilter.WithGroups(groups...)
		}
		if len(filter.Statuses) > 0 {
			var statuses []coredata.IcpmsDocumentStatus
			for _, s := range filter.Statuses {
				statuses = append(statuses, coredata.IcpmsDocumentStatus(s))
			}
			coreFilter.WithStatuses(statuses...)
		}
		if len(filter.Priorities) > 0 {
			var priorities []coredata.IcpmsDocumentPriority
			for _, p := range filter.Priorities {
				priorities = append(priorities, coredata.IcpmsDocumentPriority(p))
			}
			coreFilter.WithPriorities(priorities...)
		}
		if len(filter.Applicabilities) > 0 {
			var applicabilities []coredata.IcpmsDocumentApplicability
			for _, a := range filter.Applicabilities {
				applicabilities = append(applicabilities, coredata.IcpmsDocumentApplicability(a))
			}
			coreFilter.WithApplicabilities(applicabilities...)
		}
	}

	pageOrderBy := page.OrderBy[coredata.IcpmsDocumentOrderField]{
		Field:     coredata.IcpmsDocumentOrderFieldCreatedAt,
		Direction: page.OrderDirectionDesc,
	}

	if orderBy != nil {
		pageOrderBy = page.OrderBy[coredata.IcpmsDocumentOrderField]{
			Field:     coredata.IcpmsDocumentOrderField(orderBy.Field),
			Direction: page.OrderDirection(orderBy.Direction),
		}
	}

	cursor := types.NewCursor(first, after, last, before, pageOrderBy)

	documentsPage, err := r.probo.IcpmsDocuments.ListForOrganizationID(ctx, scope, obj.ID, cursor, coreFilter)
	if err != nil {
		r.logger.ErrorCtx(ctx, "cannot list icpms documents", log.Error(err))
		return nil, gqlutils.Internal(ctx)
	}

	return types.NewIcpmsDocumentConnection(documentsPage, r, obj.ID, coreFilter), nil
}
