...
func NewCmdPublish(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "publish",
		Short: "Publish the processing activity register as a document version",
		Example: `  # Publish the processing activity register
  prb processing-activity publish --org ORG_ID

  # Publish with approvers
  prb processing-activity publish --org ORG_ID --approver PROFILE_ID1 --approver PROFILE_ID2`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			v := resp.PublishProcessingActivityList.DocumentVersionEdge.Node
			_, _ = fmt.Fprintf(
				f.IOStreams.Out,
				"Processing activity register %s v%d.%d (%s)\n",
				v.Title,
				v.Major,
				v.Minor,
				v.Status,
