...
func NewCmdPublish(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "publish",
		Short: "Publish the Transfer Impact Assessment register as a document version",
		Example: `  # Publish the TIA register
  prb tia publish --org ORG_ID

  # Publish with approvers
  prb tia publish --org ORG_ID --approver PROFILE_ID1 --approver PROFILE_ID2`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			v := resp.PublishTransferImpactAssessmentList.DocumentVersionEdge.Node
			_, _ = fmt.Fprintf(
				f.IOStreams.Out,
				"Transfer Impact Assessment register %s v%d.%d (%s)\n",
				v.Title,
				v.Major,
				v.Minor,
				v.Status,
